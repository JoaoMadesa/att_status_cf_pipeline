...
# ===================== CONFIG via ENV =====================
# Confirma Fácil
CF_EMAIL       = "j.rhoden@madesa.com"  # Substitua com o seu e-mail
CF_SENHA       = "Jvwr2007@"          # Substitua com a sua senha
...
PAGE_SIZE  = int(os.getenv("PAGE_SIZE", "500"))  # Reduzido para 500
MAX_WORKERS = int(os.getenv("MAX_WORKERS", "5"))  # Reduzido para 5
TIMEOUT    = (5, 120)
TOTAL_RETRIES = 3
BACKOFF      = 2  # Ajustado para um backoff exponencial mais forte
...
def fetch_page(session, token, params):
    time.sleep(2)  # Delay entre as requisições
...
def format_data_iso_to_br(iso_str: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logging.error(f"Falha ao formatar data: {iso_str}")
            return ""
...
def extrair_df(respostas) -> pd.DataFrame:
    ...
    for item in respostas:
        ...
        if not emb:
            logging.warning("Embarque não encontrado.")
            continue
    ...
    if df.empty:
        logging.warning("Nenhum dado extraído da resposta.")
    return df
    ...
    for nome, cod in CODES.items():
        ...
        try:
            ...
            if not d.empty:
                ...
                logging.warning(f"[INFO] {nome}: Nenhum dado encontrado.")
        except Exception as e:
            logging.error(f"[ERROR] Falha ao processar {nome}: {e}")
...
def update_status(_output_dir: Path):
    ...
    for status, p in file_paths.items():
        try:
            ...
            logging.error(f"[ERROR] Falha ao processar arquivo {p}: {e}")
...
def run_pipeline(lookback: int, clear_first: bool = True):
    ...
    if clear_first: clear_google_sheet()
